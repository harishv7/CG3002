...
class Constant:
	...
	DOORS = [
	(1, 2, 10, 11), (1, 2, 15, 32), (1, 2, 34, 26), (1, 2, 26, 29), 
	(1, 2, 11, 10), (1, 2, 32, 15), (1, 2, 26, 34), (1, 2, 29, 26), 
	(2, 2, 6, 11), (2, 2, 11, 12), (2, 2, 12, 13), 
	(2, 2, 14, 13), (2, 2, 13, 12), (2, 2, 12, 11), 
	(2, 3, 2, 7), (2, 3, 7, 8), (2, 3, 8, 9), 
	(2, 3, 10, 9), (2, 3, 9, 8), (2, 3, 8, 7)
	]
...
def get_json(building_num, level_num):
	try: # Try to retrieve json online
		...
		try: # Try to load cached json
			filename = "Building" + str(building_num) + "Level" + str(level_num) + ".json"
...
def identify_intermediate_nodes(source_node_info, target_node_info):
	...
	# Check if source and target are already in the same building
	if (source_node_info[0] == target_node_info[0] and source_node_info[1] == target_node_info[1]):
		return [source_node_info, target_node_info]
	...
	connected_node_info = (None, None, None)
	...
	while (que.qsize() != 0 and not is_target_building_found):
		...
		for connected_node_info in connected_nodes_info:
			...
			if (connected_node_info[0] == current_node_info):
				...
				is_visited[current_node_info] = True
			# Check if target building is reached
...
def receive_data(port, connection_state):
	...
	computed_checksum = computed_checksum ^ int(heading_angle)
	...
	computed_checksum = computed_checksum ^ int(surface_height)
...
def main():
	...
	if (last_intermediate_node_info[0] != next_intermediate_node_info[0] or last_intermediate_node_info[1] != next_intermediate_node_info[1]):
	...
	while (connection_state == Constant.CONNECTED_STATE or connection_state == Constant.POLLING_STATE):
		...
		if (is_initial_data):
			...
			for doorInfo in Constant.DOORS:
				if (current_graph.get_building_num() == doorInfo[0] and current_graph.get_level_num() == doorInfo[1] and 
					last_node.get_id() == doorInfo[2] and next_node.get_id() == doorInfo[3]):
					os.system(Constant.ESPEAK_FORMAT.format("Caution! Door ahead"))
					print("Caution! Door ahead")
					break
			port.flushInput()
			...
			# Tolerate some difference in heading angle vs the expected heading angle
			if (math.fabs(rotate_direction) <= Constant.HEADING_ANGLE_TOLERANCE):
				graph_aligned_heading_angle = current_graph.calculate_graph_aligned_angle(heading_angle) + rotate_direction
				if (graph_aligned_heading_angle <= -180):
					graph_aligned_heading_angle += 360
				if (graph_aligned_heading_angle > 180):
					graph_aligned_heading_angle -= 360
			else:
				graph_aligned_heading_angle = current_graph.calculate_graph_aligned_angle(heading_angle)

			current_position = Point(
				current_position.get_x() + math.sin(graph_aligned_heading_angle * Constant.DEGREE_TO_RADIAN) * 
				(step_count - last_step_count) * Constant.AVERAGE_STEP_DISTANCE, 
				current_position.get_y() + math.cos(graph_aligned_heading_angle * Constant.DEGREE_TO_RADIAN) * 
				(step_count - last_step_count) * Constant.AVERAGE_STEP_DISTANCE
				)
